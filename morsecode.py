...
def is_validated_english_sentence(user_input):
    ...
    munjang=list('.,!?')
    for s in user_input:
        if s.isdigit() or s in punc_list:
            return False
        if s not in munjang:
            temp+=s
...
def is_validated_morse_code(user_input):
    ...
    if user_input.isspace():
        return False
    one_words=user_input.split()
    for one_word in one_words:
        morse_dict=get_morse_code_dict()
        if one_word not in morse_dict.values():
            return False
        for s in one_word:
...
def encoding_character(english_character):
    ...
    result = None
    for char,morse in morse_code_dict.items():
        if english_character.upper()== char:
            return morse
    # ==================================
...
def decoding_sentence(morse_sentence):
    ...
    for morse_word in morse_sentence_list:
        morse_word_list=morse_word.split()
        for morse in morse_word_list:
...
def encoding_sentence(english_sentence):
    ...
    clean_english_sentence_list=clean_english_sentence.split()
    for morse_word in clean_english_sentence_list:
        for morse in morse_word:
            result+=(encoding_character(morse)+' ')
        result+=' '
    return result.strip()
    # ==================================
